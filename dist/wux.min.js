...
break
case t.WInputType.Ftp:var A='<input type="text" name="'+p.id+'" id="'+p.id+'" class="'+this.inputClass+'" '
p.readonly&&(A+="readonly "),0==p.enabled&&(A+="disabled "),p.style&&(A+=' style="'+t.style(p.style)+'"'),A+="/>",p.element=$(A)
break
case t.WInputType.Password:var E='<input type="password" name="'+p.id+'" id="'+p.id+'" class="'+this.inputClass+'" '
p.readonly&&(E+="readonly "),0==p.enabled&&(E+="disabled "),p.style&&(E+=' style="'+t.style(p.style)+'"'),E+="/>",p.element=$(E)
break
case t.WInputType.Cron:var k=""
null!=p.value&&""!=p.value&&(k='initial: "'+p.value+'" ,')
var N='<div id="cron_'+p.id+'" class="'+this.inputClass+'" '
p.readonly&&(N+="readonly "),0==p.enabled&&(N+="disabled "),N+=" ></div>",N+='<script type="text/javascript">$(document).ready(function() {',N+='$("#cron_'+p.id+'").cron({'+k+'onChange: function() {$("#'+p.id+'").val($(this).cron("value"));},useGentleSelect: true});',N+='});<\/script><input type="hidden" name="'+p.id+'" id="'+p.id+'" />',p.element=$(N)
break
...
if(this.footerClass||(this.footerClass="col-md-12"),U=U?' style="'+U+'"':' style="text-align:right;"',this.footerSep)if("string"==typeof this.footerSep){var F=this.footerSep.charAt(0)
"<"==F?this.root.append(this.footerSep):t.WUtil.isNumeric(F)&&this.root.append('<div style="height:'+this.footerSep+'px;"></div>')}else this.root.append('<div style="height:'+this.footerSep+'px;"></div>')
var P=$('<div class="row"></div>')
this.root.append(P)
var H=$('<div class="'+this.footerClass+'"'+U+"></div>")
P.append(H)
for(var B=0,V=this.footer;B<V.length;B++){var j=V[B]
"string"==typeof j&&j.length>0?H.append($(j)):j instanceof t.WComponent?j.mount(H):H.append(j)}}for(var X=this,G=0,Y=this.dpids;G<Y.length;G++){var K=Y[G]
!function(n){var r=t.WUtil.getDate(X.minValues,n)
...
c>=19000101&&u>c&&(t.showWarning(t.RES.ERR_DATE),$(r.target).datepicker("setDate",l))}}}})}(K)}"object"==typeof this.state&&this.updateView(),this.root.find("input").keypress(function(t){if(13==t.which){for(var n=$(t.target).attr("id"),r=e.next(n);r&&!e.isFocusable(r);)r=e.next(r.id)
